...
export const createLeave = async (req, res) => {
  try {
    ...
    const { emp_id, type, startDate, endDate, reason, shift_id, duration } =
      value;
...
export const getLeaves = async (req, res) => {
  try {
    ...
    const leaves = await Leave.find({ deleted: 0 })
      .populate("emp_id", "fullname email")
      .populate("shift_id", "name startTime endTime")
      .populate("attendanceLink")
      .sort({ createdAt: -1 })
  ...
  try {
    ...
    if (value.startDate || value.endDate || value.emp_id) {
      const attendanceRecords = await Attendance.find({
        ...
        date: {
          $gte: value.startDate || leave.startDate,
          $lte: value.endDate || leave.endDate,
        },
      });
