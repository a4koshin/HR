        ...
        <div className="mb-10 flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="mb-6 sm:mb-0">
            ...
            <p className="text-gray-600 mt-3 text-lg">
              Manage organizational departments and teams efficiently
            </p>
          </div>
          <button
            onClick={openAddModal}
            disabled={isLoading}
            className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-blue-400 disabled:to-blue-400 text-white px-6 py-3.5 rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center gap-3 min-w-[180px]"
          >
            {isLoading ? (
          ...
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {departments.map((dep) => (
                    <tr key={dep._id} className="hover:bg-gray-50 transition-all duration-200 group cursor-pointer">
                      <td className="px-8 py-5">
                        <div className="text-gray-900 font-semibold">{dep.name}</div>
                        {dep.description && (
                          <div className="text-gray-500 text-sm truncate max-w-xs">{dep.description}</div>
                        )}
                      </td>
                      <td className="px-6 py-5">
                        ...
                        >
                          <div className={`w-2 h-2 rounded-full ${dep.status === "Active" ? "bg-green-500" : "bg-red-500"}`}></div>
                          {dep.status}
                        </span>
                      </td>
            ...
            {departments.length === 0 && !isLoading && (
              <div className="text-center py-16">
                ...
                <p className="text-gray-600 mb-6">Get started by adding your first department</p>
                <button
                  onClick={openAddModal}
