...
const DashboardPage = () => {
  ...
  const { data: empData = [], isLoading: empLoading, isError: empError } =
  ...
  const { data: payrollData = [], isLoading: payrollLoading, isError: payrollError } =
    useGetallFunctionQuery({ url: "/payrolls" });

  const { data: attendanceData = [], isLoading: attLoading, isError: attError } =
  ...
  const attendanceRecords = attendanceData?.attendances || attendanceData?.data || [];
  ...
  const totalDeductions = payrolls.reduce((sum, p) => sum + (p.deduction || 0), 0);
  const paidCount = payrolls.filter((p) => p.paidStatus === "Paid").length;
  const pendingPayrolls = payrolls.filter((p) => p.paidStatus === "Pending").length;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Error loading dashboard data</h3>
...
const MetricCard = ({ title, value, subtitle, icon, color, trend, compact = false }) => {
  const colorClasses = {
  ...
  return (
    <div className={`bg-white rounded-2xl p-${compact ? '4' : '6'} shadow-lg border border-gray-100 hover:shadow-xl transition-all duration-200 group hover:translate-y-[-2px]`}>
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <p className="text-sm font-medium text-gray-600">{title}</p>
          <p className={`text-${compact ? '2xl' : '3xl'} font-bold text-gray-900 mt-1`}>{value}</p>
          {subtitle && (
        ...
        <div className={`p-3 rounded-xl ${bg} ${text} group-hover:scale-110 transition-transform duration-200`}>
          {icon}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm font-semibold text-gray-900">{value} / {total}</span>
      </div>
